  ...
  const rawData = Object.values(data || {}).flat().filter(row => row?.gross !== undefined);
    ...
    if (view === 'monthly') {
      ...
      return Object.values(monthMap)
        .sort((a, b) => {
          // Parse the month and year from the monthSort string
        ...
        .map(entry => ({
          ...entry,
          occupancyRate: Math.round((entry.nights / (entry.daysInMonth * 2)) * 100) // Assuming two apartments
        }));
    }
    ...
    ).sort((a, b) => a.year - b.year) // Sort years numerically
    .map(entry => ({
      ...entry,
      occupancyRate: Math.round((entry.nights / (366 * 2)) * 100) // Assuming two apartments, and 366 days to handle leap years
    }));
  };
